...
describe('HeatmapTracker — proximity click signals', () => {
  let tracker: HeatmapTracker;
  ...
  it('single click on button is interactive but not dead', () => {
    const batcher = makeBatcher();
    tracker = makeTracker(batcher);
  ...
  it('flags dead click on second nearby click on same button', () => {
    vi.restoreAllMocks();
    const batcher = makeBatcher();
    tracker = makeTracker(batcher);
    ...
    const events = batcher.pushed as Array<{ metadata: Record<string, unknown> }>;
    expect(events[0].metadata.is_dead_click).toBe(false);
    expect(events[1].metadata.is_dead_click).toBe(true);
  });

  it('pointer-styled div without DOM cues is not interactive on first click', () => {
    const batcher = makeBatcher();
    tracker = makeTracker(batcher);
  ...
  it('role=button is interactive immediately', () => {
    const batcher = makeBatcher();
    tracker = makeTracker(batcher);
  ...
  it('flags rage and dead on third rapid nearby click', () => {
    vi.restoreAllMocks();
    vi.spyOn(Math, 'random').mockReturnValue(0);
    const batcher = makeBatcher();
    tracker = makeTracker(batcher);
